      ...
      <div className="max-w-4xl mx-auto bg-white/60 backdrop-blur-md p-6 rounded-3xl shadow-2xl">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 gap-4">
            {documents.map((doc) => (
              <div key={doc._id} className="bg-white rounded-xl p-4 shadow hover:shadow-md transition-shadow">
                <h2 className="text-xl font-semibold text-indigo-600">{doc.title}</h2>
                <p className="text-sm text-gray-500 mt-1">
                  Saved: {new Date(doc.createdAt).toLocaleString()}
                </p>
                <div className="mt-3 flex gap-6">
                  <Link to={`/editor/${doc._id}`} className="text-indigo-700 font-medium underline hover:text-indigo-900">
                    Open in Editor
                  </Link>
                  <button onClick={() => handleDelete(doc._id)} className="text-red-600 font-medium hover:underline">
                    Delete
                  </button>
                </div>
              </div>
